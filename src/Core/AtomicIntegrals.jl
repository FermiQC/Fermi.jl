...
function unique_ao_4c2e(BS::BasisSet, T::DataType = Float64)
    ...
    # Allocate output array
    shell_idx = find_indices(BS.nshells)
    bas_idx = find_index(BS.nbas)
    out = zeros(T, length(bas_idx))

    test = zeros(Threads.nthreads())
    x1 = 1
    @sync for (i,j,k,l) in shell_idx
        Threads.@spawn begin
            @inbounds begin
                # Shift indexes (C starts with 0, Julia 1)
                id, jd, kd, ld = i+1, j+1, k+1, l+1

                buf_length = num_prim[id]*num_prim[jd]*num_prim[kd]*num_prim[ld]
                # Initialize array for results
                buf = ones(Cdouble, buf_length)

                # Compute ERI
                cint2e_sph!(buf, Cint.([i,j,k,l]), BS.lc_atoms, BS.natoms, BS.lc_bas, BS.nbas, BS.lc_env)

                if i != j && k != l && index2(i,j) != index2(k,l)
                    out[x1:x1+buf_length-1] .= buf

                elseif k != l && index2(i,j) != index2(k,l) # i == j
